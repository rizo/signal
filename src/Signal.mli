...
val null : unit t
(** [null] is a constant signal of [()]. It ignores it's subscribers and ignores
    values emitted to it. *)

(** {2 Notifications}

    Notifications control the propagation of signal values to subscribers. The
    value emitting functions such as {!val:emit} and {!val:update} accept an
    optional [~notify] argument which changes the propagation strategy. For
    example, it is possible to fully prevent notifying the subscribers by using
    [Signal.emit ~notify:Signal.never] and later call {!val:Signal.trigger} to
    notify the subscribers at a more appropriate time. *)
...
(** [scope f] calls [f] with a notification that can be used to batch notify all
    subscribers for emits that happened in [f]'s scope. The current values of
    the signals are still changed immediately. *)

(** {2 Updating, getting and propagating values} *)

val emit : ?notify:notification -> 'a -> 'a t -> unit
(** [emit ?notify x s] changes the value of signal [s] to [x]. The emitted value
    will be dispatched to all subscribers of [s] according to [notify] (defaults
    to {!val:now}). *)
...
(** [update ?step f s] changes the current value of [s] using [f], emitting the
    result. *)
...
(** [use' f s] subscribes to [s] and immediately calls the callback [f] with the
    current value of the signal. Returns a subscription handle that can be used
    to unsubscribe [f] from the signal [s]. *)
...
(** [sub' f s] subscribes to [s] without immediately calling [f], instead [f] is
    called when new values are emitted to [s]. Returns a subscription handle
    that can be used to unsubscribe [f] from the signal [s]. *)
...
val t5 :
  'a1 t -> 'a2 t -> 'a3 t -> 'a4 t -> 'a5 t -> ('a1 * 'a2 * 'a3 * 'a4 * 'a5) t
...
(** [sample ?equal ~on:s1 s2] samples values from [s2] any time a value is
    emitted to [s1]. *)
